...
if($page<$tpage)
{
    echo '<a href="qna.php?page='.($page+1).'" class="next-btn" id="next-btn"><input type="submit" name="next" value="Next" onclick="arry()"></a>';
} 
?>
</form>
  </div> 
...
function arry()
{
var questiontop=<?Php echo $question; ?>;
var answertop=<?php echo $answer; ?>;
 const question=new Array(<?php echo $tpage; ?>);
 const answer=new Array(<?php  echo $tpage;?>);
let page=<?php echo $page;?>;
let tpage=<?php echo $tpage;?>;
for(var j=page;j<tpage;j++ )
{
    question[j]=questiontop;
    answer[j]=answertop;
}
const hello=document.getElementsById('gfg');
hello.submit();
}
 </script>
    </body>
